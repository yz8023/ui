...
- **七套主题配色**：冰蓝 / 薄荷 / 落日 / 素灰 / 珊瑚 / 樱花 / 星夜，设置页色卡即点即换，`Theme.kt` 的 `PaletteSeeds` 一处维护。
- **真玻璃弹窗**：弹窗改为与页面同窗渲染，采样 aurora+content 合成层，带弹簧缩放 + 淡入入场动效。
- **桌面壁纸背景**：设置页开关（首次开启请求媒体权限），直接以手机桌面壁纸作为玻璃折射的背景层。
- **弹簧选择立即回弹**：动效实验室点选预设即触发一段该阻尼的回弹演示，效果即刻可见。
- **全局动效阻尼滑块**：一个滑块重调全应用弹簧阻尼（token 级倍率），越低越弹。
- **开关组件可见性**：轨道放大、加描边与阴影，拇指竖直居中，明暗两态区分更清晰。
- **设计参考署名**：设置页「关于」列出参考仓库 dyparse / Miuix / motion 与地址。

> v1.1.1：修复壁纸背景在 API 31+ 缺少媒体权限时的崩溃（自动降级 aurora + 运行时授权）、
> 优化暗色主题中性色、新增珊瑚/樱花/星夜三套风格。

## v1.2 · 更新内容

- **控件可拖拽 + 材质自适应**：开关 / 分段控件的滑块支持水平拖动（松手弹簧吸附），
  统一走液态玻璃材质并随「玻璃 / 普通 / MD3」三态自动降级。
- **日夜模式平滑过渡**：主题切换时全部配色角色交叉淡化，不再闪眼。
- **自定义背景与配色**：「自定义」色卡 + 圆形 HSV 色盘取色（点击/拖动无极调色）+ 明度滑条 + 深浅滑块，自由配色即时生效。
- **通知预览**：设置页一键发送一条使用当前主题主色着色的系统通知。
- **切屏弹性动效**：页面切换带弹簧缩放 + 淡入的反弹入场。
